...
class GVSNet(nn.Module):
    ...
    def forward(self, input_data, mode='generator'):
        if mode == 'generator':
            ...
            with torch.no_grad():
                color_nv, sem_nv, disp_iv, _kld_loss, sem_loss, disp_loss = self.generate_fake(input_data)
            return color_nv
        else:
            raise KeyError('')
    ...
    def _get_scene_encoding(self, input_img):
        ...
        if self.opts.mode == 'train':
            z, mu, logvar, feature_List = self.encoder(input_img)
    ...
    def generate_fake(self, input_data):
        num_layers = self.opts.num_layers
        height, width = self.opts.img_wh[0], self.opts.img_wh[1]
        feats_per_layer = self.opts.feats_per_layer
        batch_size = input_data['input_seg'].shape[0]
        # Get style encoding of the scene
        z, mu, logvar, feature_List = self._get_scene_encoding(input_data['style_img'])
        # Infer scene semantics and geometry
        ...
        layered_sem = layered_sem.flatten(1, 2)
        layered_appearance = self.spade_ltn(layered_sem,feature_List, z=z).view(
            batch_size, num_layers, feats_per_layer, height, width)
        mpi_appearance = self.apply_association(
            layered_appearance, input_associations=associations)
        ...
        mpi_sem_nv, _ = self.apply_homography(
            h_matrix=h_mats, src_img=mpi_sem, grid=grid)
        mpi_app_nv, _ = self.apply_homography(
            h_matrix=h_mats, src_img=mpi_appearance, grid=grid)
        appearance_nv = self.alpha_composition(
            src_imgs=mpi_app_nv, alpha_imgs=mpi_alpha_nv)
        ...
        semantics_loss = self.compute_semantics_loss(
            sem_nv, input_data['target_seg'])

        disp_iv = self.alpha_to_disp(
            mpi_alpha, input_data['k_matrix'], self.opts.stereo_baseline, novel_view=False)
        disp_loss = self.opts.disparity_weight * F.l1_loss(disp_iv, input_data['input_disp'])

        color_nv = self.adn(appearance_nv)
        kld_loss = self.get_kld_loss(mu, logvar) * self.opts.lambda_kld
        return color_nv, sem_nv, disp_iv, kld_loss, semantics_loss, disp_loss
    ...
    def compute_discriminator_loss(self, input_data):
        ...
        D_losses = {}
        with torch.no_grad():
            fake_image, sem_nv, disp_iv, _kld_lossm, sem_loss, disp_loss = self.generate_fake(input_data)

...
class SUNModel(torch.nn.Module):
    ...
    def forward(self, input_data, mode='inference'):
        if mode == 'inference':
            with torch.no_grad():
                ...
                # Here we do novel-view synthesis of alpha features
                ...
                alpha_nv, grid = self.apply_homography(
                    h_matrix=h_mats, src_img=alpha)

            return semantics_nv, alpha_nv, disp_nv
        elif mode == 'training':
    ...
    def _infere_scene_repr(self, input_data):
        ...
        input_seg_ = self.semantic_embedding.encode(
            input_seg) if encoding_needed else input_seg
        ...
        seg_mul_layer = torch.cat(
            [input_seg_.unsqueeze(1), seg_mul_layer], dim=1)
